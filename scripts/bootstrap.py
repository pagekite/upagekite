...
CODE_SOURCE = ('https', 'FIXME.pagekite.me', '/esp32/%(hwid)s/bootstrap.json')
...
def http_get(proto, host, path):
  ...
  client.connect(socket.getaddrinfo(host, 443)[0][-1])
  if proto == 'https':
...
def do_bootstrap(src):
  proto, host, path, result, headers, bootstrap_json = http_get(*src)
